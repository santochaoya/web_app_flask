...
from flaskblog.forms import PostForm, RequestResetForm, ResetPasswordForm
...
@app.route('/post/new', methods=['GET', 'POST', 'DELETE'])
@login_required
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        ...
        if post:
            db.session.add(post)
            db.session.commit()
            flash(f'The post has been created!', category='success')
...
def delete_post(post_id):
    ...
    db.session.delete(Post.query.all())
    db.session.commit()
